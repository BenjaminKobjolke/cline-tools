...
class DiffHandler:
    ...
    def create_temp_files(
        ...
        """
        try:
            external_tmp = os.path.join(
                self.tmp_dir, f"external_{block_type.lower()}_block.md"
            )
            local_tmp = os.path.join(
                self.tmp_dir, f"local_{block_type.lower()}_block.md"
            )
